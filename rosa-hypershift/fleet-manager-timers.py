...
#cluster_states = ["cluster_accepted", "cluster_account_provisioned", "cluster_network_provisioned", "cluster_provisioning", "cluster_network_updating", "cluster_provisioned", ]
...
def _build_cluster(ocm_cmnd, rosa_cmnd, cluster_name_seed, must_gather_all, mgmt_cluster_name, provision_shard, create_vpc, vpc_info, wait_time, cluster_load, load_duration, job_iterations, worker_nodes, my_path, my_uuid, my_inc, es, es_url, index, index_retry, mgmt_kubeconfig, sc_kubeconfig, all_clusters_installed, aws_region):
    ...
    if provision_shard != "":
        ...
        cluster_cmd.append("--region")
        cluster_cmd.append(aws_region)
    ...
    if process.returncode == 0:
        ...
        metadata['mgmt_namespace'] = 0
        metadata['sc_namespace'] = 0
        if kubeconfig == "":
        ...
        if wait_time != 0 and cluster_load:
...
def _update_mc_metadata(es, index, index_retry, uuid, mc, status_updated):
    metadata = {}
    metadata['uuid'] = uuid
    metadata['cluster_id'] = mc['id']
    if status_updated:
        metadata['status'] = mc['status']
        metadata['duration'] = int(time.time()) - mc['updated']
        metadata['metric_name'] = "sc_mc_status"
        cs = mc['status_updates']
        if cs.get('cluster_accepted'):
            cluster_start = cs.get('cluster_accepted')
        elif cs.get('cluster_account_provisioned'):
            cluster_start = cs.get('cluster_account_provisioned')
        else:
            cluster_start = cs.get('cluster_provisioning')
        if mc['status'] == "cluster_provisioned":
            metadata['provisioned'] = int(time.time()) - cluster_start
        elif mc['status'] == "ready":
            metadata['mc_ready'] = int(time.time()) - cs['cluster_provisioned']
            metadata['total'] = int(time.time()) - cluster_start
    else:
        metadata['metric_name'] = "mc_hc_count"
        metadata['hc_count'] = mc['hc_count']

    metadata["timestamp"] = datetime.datetime.utcnow().isoformat()
    metadata['cluster_name'] = mc.get('name', "")
    metadata['service_cluster'] = mc['sc']
    es_ignored_metadata = ""
    if es is not None:
        common._index_result(es, index, metadata, es_ignored_metadata, index_retry)

def _get_hc_for_sc(kubeconfig, my_path, mgmt_cluster):
    myenv = os.environ.copy()
    myenv["KUBECONFIG"] = kubeconfig
    cmd = ["oc", "get", "addonplacementscores.cluster.open-cluster-management.io", "hosted-clusters-score", "-n", mgmt_cluster, "--output", "json"]
    #oc get addonplacementscores.cluster.open-cluster-management.io hosted-clusters-score -n hs-mc-9f5sjvcd0 -o json | jq '.status.scores[0].value'
    hccount_process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=myenv)
    hccount_stdout, hccount_stderr = hccount_process.communicate()
    if hccount_process.returncode != 0:
        logging.error("Failed to get HC count for Management Cluster %s" % mgmt_cluster)
        return 0
    else:
        try:
            hccount_json = json.loads(hccount_stdout)
        except Exception as err:
            logging.error("Failed to get HC count for Management Cluster %s" % mgmt_cluster)
            return 0
        for score in hccount_json['status']['scores']:
            if score['name'] == 'hostedClustersCount':
                return score['value']
        return 0

def _run_commdand(cmd):
    logging.debug(cmd)
    ...
    try:
        ...
        logging.error("Failed to get service clusters list: %s" % err)
        logging.error(stdout)
        logging.error(stderr)
        return {}

def _mc_watcher(ocm_cmnd, my_path, cluster_name_seed, delay, my_uuid, aws_region, es, es_index, es_index_retry):
    #time.sleep(60)
    time.sleep(5)
    ...
    # ocm get /api/osd_fleet_mgmt/v1/management_clusters --parameter search="region is 'us-east-2' and status is 'ready' and parent.id is 'cfl443tbp9s50f9squrg'"
    sc_cmd = [ocm_cmnd, "get", "/api/osd_fleet_mgmt/v1/service_clusters?search=status+is+%27" + "ready" + "%27+and+region+is+%27" + aws_region + "%27"]
    serv_clusters = {}
    while True:
        process = subprocess.Popen(sc_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
        stdout, stderr = process.communicate()
        try:
            scs = json.loads(stdout)
        except ValueError as err:
            logging.error("Failed to get service clusters list: %s" % err)
            logging.error(stdout)
            logging.error(stderr)
        ...
        for sc in scs.get("items", []):
            sc_cluster_id = sc['cluster_management_reference']['cluster_id']
            serv_cluster = serv_clusters.get(sc['id'], None)
            if serv_cluster:
                if sc['status'] != serv_cluster['status']:
                    serv_cluster['status'] = sc['status']
                    duration = int(time.time()) - serv_cluster['updated']
                    serv_cluster['updated'] = int(time.time())
                    #_update_sc_mc_metadata(es, es_index, es_index_retry, sc['id'], my_uuid, sc['id'], sc['name'], sc['status'], duration, 0)
                    logging.info("SC %s status changed to %s at %s" % (sc['id'], sc['status'], datetime.datetime.utcnow().isoformat()))
                    if not serv_cluster.get('kubeconfig', None) and sc['status'] in ['ready', 'cluster_provisioned']:
                        serv_cluster['kubeconfig'] = _download_kubeconfig(ocm_cmnd, sc_cluster_id, my_path, sc['name'])
            else:
                serv_clusters[sc['id']] = {'name': sc['name'], 'status': sc['status'], 'updated': int(time.time()), 'mgmt_clusters': {}}
                #_update_sc_mc_metadata(es, es_index, es_index_retry, sc['id'], my_uuid, sc['id'], sc['name'], sc['status'], 0, 0)
                logging.info("New SC %s status changed to %s at %s" % (sc['id'], sc['status'], sc['updated_timestamp']))
                if sc['status'] in ['ready', 'cluster_provisioned']:
                    serv_clusters[sc['id']]['kubeconfig'] = _download_kubeconfig(ocm_cmnd, sc_cluster_id, my_path, sc['name']) 

            mgmt_clusters = serv_clusters[sc['id']]['mgmt_clusters']
            sc_kubeconfig = serv_clusters[sc['id']].get('kubeconfig', "")
            mc_cmd = [ocm_cmnd, "get", "/api/osd_fleet_mgmt/v1/management_clusters?search=parent.id+is+%27" + sc['id'] + "%27+and+region+is+%27" + aws_region + "%27"]
            process = subprocess.Popen(mc_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            stdout, stderr = process.communicate()
            try:
                mcs = json.loads(stdout)
            except ValueError as err:
                logging.error("Failed to get management clusters list: %s" % err)
                logging.error(stdout)
                logging.error(stderr)
                mcs = {}
            for mc in mcs.get("items", []):
                ...
                mc_name = mc.get('name', "")
                mc_status = mc['status']
                mgmt_cluster = mgmt_clusters.get(mc_id, None)
                if mgmt_cluster:
                    if mc_status != mgmt_cluster['status']:
                        # save new status
                        mgmt_cluster['status'] = mc_status
                        if mc_name != "" and mgmt_cluster['name'] == "":
                            mgmt_cluster['name'] = mc_name
                        mgmt_cluster['status_updates'][mc_status] = int(time.time())
                        _update_mc_metadata(es, es_index, es_index_retry, my_uuid, mgmt_cluster, True)
                        mgmt_cluster['updated'] = int(time.time())
                        logging.info("MC %s status changed to %s at %s mc name is %s" % (mc_id, mc_status, datetime.datetime.utcnow().isoformat(), mc_name))
                else:
                    # add new mc entry
                    status_updates = {mc_status: int(time.time())}
                    mgmt_clusters[mc_id] = {'id': mc_id, 'name': mc_name, 'sc': sc['id'], 'status': mc_status, 'updated': int(time.time()), 'status_updates': status_updates}
                    # Avoid metrics collection for existing MCs
                    if mc_status == "cluster_accepted":
                        _update_mc_metadata(es, es_index, es_index_retry, my_uuid, mgmt_clusters[mc_id], True)
                    logging.info("New MC %s status changed to %s at %s mc name is %s" % (mc_id, mc_status, mc['updated_timestamp'], mc_name))

                # update hc count
                mgmt_cluster = mgmt_clusters.get(mc_id, None)
                if mgmt_cluster and sc_kubeconfig != "" and mc_status == 'ready':
                    hc_count = _get_hc_for_sc(sc_kubeconfig, my_path, mc_name)
                    if ('hc_count' not in mgmt_cluster.keys()) or (hc_count != mgmt_cluster['hc_count']):
                        mgmt_cluster['hc_count'] = hc_count
                        logging.info("HC count changed from %s to %s for MC %s " % (mgmt_cluster['hc_count'], hc_count, mc_name))
                        _update_mc_metadata(es, es_index, es_index_retry, my_uuid, mgmt_cluster, False)
...
def main():
    ...
    parser.add_argument(
        '--provision-shard',
        type=str,
        help='Provision Shared ID')
    parser.add_argument(
        '--run-duration',
        type=int,
        default=1,
        help="Fleet manager timers watcher duration in minutes")
    parser.add_argument(
        '--fm-es-index',
        type=str,
        help='ES index for FM metrics')
    parser.add_argument(
        '--mgmt-org-id',
        type=str,
...
# Delete following parameter and code when default security group wont be used
    ...
    access_to_service_cluster = False
    mgmt_kubeconfig_path = ""
    sc_kubeconfig_path = ""
    if args.provision_shard:
        provison_shard = args.provision_shard
    else:
        provison_shard = ""
    if args.mgmt_cluster and args.mgmt_org_id:
        ...
        provison_shard = mgmt_metadata['provision_shard']
    ...
    if args.service_cluster and args.mgmt_org_id:
    ...
    logging.info('Launching mc watcher thread')
    clusters_resume = {}
    mc_watcher = threading.Thread(target=_mc_watcher, args=(ocm_cmnd, my_path, cluster_name_seed, args.watcher_delay, my_uuid, args.aws_region, es, args.fm_es_index, args.es_index_retry))
    mc_watcher.daemon = True
    mc_watcher.start()
    time.sleep(args.run_duration*60)
