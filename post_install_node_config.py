...
def _label_workload_machine(nodes):
    worker_list = nodes.get(label_selector='node-role.kubernetes.io/worker').attributes.items
    body = {
        "metadata": {
            "labels": {
...
def _remove_worker_label(nodes):
    infra_list = nodes.get(label_selector='node-role.kubernetes.io/infra').attributes.items
    body = {
        "metadata": {
            "labels": {
                "node-role.kubernetes.io/worker": None}
        }
    }
